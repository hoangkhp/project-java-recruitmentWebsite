...
public class PermissionController {
    ...
    public ResponseEntity<Permission> create(@Valid @RequestBody Permission p) 
        ...
        return ResponseEntity.ok().body(this.permissionService.handleSavePermission(p));
    }
    @PutMapping("/permissions")
    @ApiMessage("Update a permission")
    public ResponseEntity<Permission> update(@Valid @RequestBody Permission p) throws IdInvalidException {
        // check exist by id
        if (this.permissionService.fetchAPermission(p.getId()) == null) {
    ...
    public ResponseEntity<Void> delete(@PathVariable("id") long id) throws IdInvalidException {
        // check exist by id
        if (this.permissionService.fetchAPermission(id) == null) {
